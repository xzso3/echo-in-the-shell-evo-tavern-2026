...
3. Play 后先听启动音乐；靠近记忆节点或 NPC 按 E 听互动声；被敌人实际命中听受伤声；击败普通敌人听击败声。打开 Console 确认没有 `Native audio: assign a complete run and all four clips.` 报错。当前已在 `NativeDemo` 根级接入一个实例并核对四段引用；Play/Windows 听感仍待人工试玩，不能据此把 B6 记为通过。
...
音频系统只读取现有事件与状态，不修改 Quest、Combat、Map 或 Narrative 的权威状态。场景接线的 Unity 编译与引用检查已完成；Windows 构建和人工听感由用户验收。
