...
从出生点北上至 `(-10, 9)`，穿过原外围墙的新开口进入北侧区块 1。两块 `NorthRouteChunk` 实例使用同一预制体，南北端口在 `(-10, 19)` 相接；第一块的开关位于 `(-10, 16.5)`，靠近按 E 后 Map 解除接缝处的实体门，可以走进区块 2，也可原路返回。区块边墙、门均有 `BoxCollider2D` 和 `NativeObstacle`，门的开关状态仅由 `NativeMap` 持有。原三记忆、中继、Boss 和最终节点仍走东侧路线。第二块中央 `(-10, 24)` 留给后续敌人接线。
