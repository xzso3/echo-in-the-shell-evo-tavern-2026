...
进入北侧区块 2 后，在 `(-12, 22)` 的紫色「定向脉冲」终端旁按 E，点击 **执行脉冲** 或 **拒绝**。执行会通过 Combat 对仍存活的 ArcSentry 造成最多 48 点真实伤害；实际命中后才由 Narrative 记录一次行为及同步度 +20，不共享记忆。拒绝、离开终端后确认、敌人已被击败或关卡已结束都不会扣费；失败后可在条件恢复时再选。成功后打开手机 **通讯 → 本局记录**，或 **网络 → 本局行为记录**，可查看命中和代价；终端本局不再执行第二次。原医疗支援和弱点解析仍在手机支援页，四结局阈值仍为同步度／差异度各 60。

终端已接入当前场景；`Echo → Native → Connect P2 North Pulse Support` 是一次性作者工具，已接线场景不要重复执行。Unity 2021 batchmode 接线／编译日志为 `/private/tmp/p2-08-connect.log`，聚焦 Play Mode 日志为 `/private/tmp/p2-08-smoke.log`：拒绝、离开范围确认、真实受击、重复保护、原支援与结局阈值、手机记录均通过。检查使用显式定位和按钮回调；未进行人工键鼠路线、完整结局或 Windows 构建／试玩。
...
手机文字区域可滚轮/拖动阅读；通讯页在安全节点有自由文本草稿框，其他位置只显示本地主题。

- **通讯**：任务、记忆、指挥官身份、授权说明和真实本局记录始终保留固定本地对白；自由文本只尝试连接所配置的代理，闲聊不计分。收集私人记忆后可明确选择 **保留异议**，保护被系统摘要删除的异议。
...
当前 UI 为简体中文与 FusionPixel 字体，角色仍使用既有静态图；新生以简单 Canvas 方块拳和裂线表示，尚非精美动画。行为记录仅属当前运行，重开会清空，未声称跨进程保存或全服提交。完整人工键鼠、滚动手感、约五分钟节奏和最终 Windows 构建由后置集中试玩确认；非阻断表现问题留给该轮反馈。

## P2-09 指挥官自由文本切片

出生点北侧本地档案员旁的青色「安全通讯 / Tab」节点允许自由文本草稿。靠近节点，按 Tab 打开手机的通讯页，点击输入框并用 **发送** 按钮提交。离开节点、敌人靠近或关卡不在进行中时，输入焦点与在途请求会取消，草稿保留。编辑时原有输入路由屏蔽角色移动、Space、E、Tab 和 Enter；手机其余本地主题与支援操作仍按原规则工作，战斗不暂停。

客户端只调用 `Assets/StreamingAssets/commander-proxy.json` 中的代理 URL。默认 URL 为空，故当前版本**没有真实在线回复**；可继续点击任务、记忆、身份、授权等预设主题。在线回复仅显示为通讯纯文本，不会授予支援、开门、改任务或结局。配置、受限 HTTP 契约、服务端凭据边界和实网前置条件见 [P2-09 代理契约](../../Docs/Framework/Phase2/P2_09_COMMANDER_PROXY.md)。Unity 2021 batchmode 场景接线与编译日志 `/private/tmp/p2-09-connect-escalated.log` 退出 0；聚焦 Play Mode `/private/tmp/p2-09-smoke-graphics.log` 检查了安全范围、输入焦点、草稿、敌近中断与离线预设主题，未调用真实服务，未进行 Windows 构建或人工试玩。首个 `-nographics` 截图探针在调用相机渲染时触发 Unity 原生崩溃，证据 `/private/tmp/p2-09-mono-crash-nographics.json`；改用图形批处理后相同功能检查及 `/private/tmp/p2-09-phone.png` 截图通过。临时探针和崩溃文件未加入仓库。
