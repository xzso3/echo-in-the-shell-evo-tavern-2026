...
namespace Echo.NativeGame
{
    public sealed class NativeProjectile : MonoBehaviour
    {
        ...
        public void Launch(Vector2 value, float amount, NativeRunController owner)
        {
            direction = value; damage = amount; run = owner;
            transform.rotation = Quaternion.Euler(0, 0, Mathf.Atan2(value.y, value.x) * Mathf.Rad2Deg);
            Destroy(gameObject, lifetime);
        }
        void FixedUpdate()
        {
            if (!run || !run.Running) { Destroy(gameObject); return; }
            ...
            foreach (var hit in Physics2D.CircleCastAll(transform.position, radius, direction, distance))
            {
                ...
                if (enemy && enemy.Alive) { enemy.Hit(damage); Destroy(gameObject); return; }
            }
