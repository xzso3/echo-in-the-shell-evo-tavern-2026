...
更新：2026-09-23 11:34（UTC+8）。首次建账时所有 KT、NB、W0 与 INT-LT 均为**未执行**；此前 P2 工作暂停。本轮输入基线和用户改动见 [BASELINE.md](BASELINE.md)。需求文档定义产品范围，[DEVELOPMENT_PLAN.md](DEVELOPMENT_PLAN.md) 为工程基线；当前只要求 [ACCEPTANCE.md](ACCEPTANCE.md) 的 S1～S4，其他矩阵后置。
...
- 统一目标 `gpt-6-sol`／`xhigh`／请求级 `priority`。`M/T/P=待核` 表示任务尚未创建；创建后从该任务的有效 `thread_settings` 核验，不以 prompt 或用户级配置代替证据。若不是 priority，停止该任务的实现并报告。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| KT-00 | 未执行 | — / — | 主控文档；`7e274fa` | 主控已核 / 执行待核 | 汇总 W0 只读报告，冻结 `DECISIONS.md`；无实现改动 | 仅协调 | G0 技术冻结 |
| W0-A 战斗／宿主 | 未执行 | — / — | 待建／待核／`7e274fa` | 待核 | 只读 Native 战斗、Prefab 参数、Boss 依赖；原型探针须隔离 | 不占，必要探针排队 | KT-00 战斗证据 |
| W0-B 地图／物理 | 未执行 | — / — | 待建／待核／`7e274fa` | 待核 | 只读 Tilemap、碰撞、素材、UPM、1 格净空；原型探针须隔离 | 不占，必要探针排队 | KT-00 空间证据 |
| W0-C 叙事／打包／Skill | 未执行 | — / — | 待建／待核／`7e274fa` | 待核 | 只读任务、叙事、身份、导出与 Skill 分发；不改全局配置 | 不占 | KT-00 接口证据 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| KT-01 | 未执行 | — / — | 待建／待核／待集成 SHA | 待核 | `Assets/EchoLevelToolkit/Runtime/Foundation/`、asmdef 提案；KT-00 | 排队 | G1 公共 API |
