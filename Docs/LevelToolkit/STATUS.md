...
- 当前主目录文档安全点为 `6aa067c`；INT-LT 隔离分支已到 `92fd1bb`，尚未回写主目录。KT-01～10 与 NB-01～04 源码已接入隔离分支；S2 短流程已通过，三份原始 ZIP 已生成，S3 干净工程接收与 S4 正式新场景未执行。输入需求/方案文档在主目录尚未提交，执行任务须从主目录**只读**读取指定绝对路径。
...
| INT-LT | S2 已通过；三 ZIP 已产；S4 待接收作品 | `01a0cc5c-97c1-7070-9f78-e5036a812954` / local | `8003`／`codex/level-toolkit-integration`；隔离 HEAD `92fd1bb`，主目录文档 `6aa067c` | Sol/xhigh/priority 已核 | NativeDemo 受影响短 Play 已通过；KT-01～10、NB-01～04/KT-07 源码已导入。S2 真实短流程有日志。用户改手工文件夹 ZIP，三份实际 ZIP 已在 `Deliverables/LevelToolkit/0.1.0-dev.1/` 生成；未做干净工程接收/编译/启动。正式场景待真实接收作品 | 唯一使用者 | 用户 S3 ZIP 接收→S4 |
...
| KT-08 | 原 Unity 包代码已集成；极简 ZIP 手册待交 | `01a0cc80-8b2e-7e70-b63e-16c724d63b40` / local | `d36a`／`codex/kt-08-folder-zip`／`66e5a49`；原包 `56e79ef`→`8286660`→`809991d`；ZIP 扩展 guard 提交 `f33c55f`/`d92bab3` **不集成** | Sol/xhigh；本次 default 已核 | 原 Manifest/指纹等功能保留在源码，但用户最新要求本轮不运行 SHA/receipt；只交极简手工 ZIP/.meta/无覆盖说明，不自动覆盖 | 不占 Unity | 手册→S3 |
...
| S3 一次 ZIP 文件夹往返 | 三 ZIP 已产；接收未执行，未通过 | `Deliverables/LevelToolkit/0.1.0-dev.1/` 中工具包 ZIP 210 条目、完整 Level ZIP 39 条目、单项 Chunk ZIP 13 条目；均含 `Assets/` 层级和 `.meta`，未做 SHA/receipt。用户须在干净 Unity 2021.3.27f1c2 工程手工复制 Toolkit/Level、编译/启动并安装五 Skill；只生成 ZIP 不算通过，同 WorkKey Chunk 不与完整 Level 在同一工程叠加 |
...
- 后续顺序：INT-LT 只集成极简 ZIP 手册 → 用户在干净 Unity 2021.3.27f1c2 工程手工接收现有工具包/完整作品 ZIP、编译/启动、安装 Skill → INT-LT 从实际接收作品保存新增正式场景、执行 S4 → 形成主目录安全点。源分支各自保留，不能带旧祖先或覆盖用户脏文件。
- 最大剩余产品缺口：NB-01～04 正式多实例输入/叙事尚未在新增场景实跑；三 ZIP 已产但未在干净工程接收，五 Skill 未发现；S3/S4 尚未完成。S2 的物理键盘 E 未直接测。不得把当前安全点称为工具包快速版交付。
