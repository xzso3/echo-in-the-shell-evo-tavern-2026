...
更新：2026-09-23 11:48（UTC+8）。本文件汇总 W0 只读核验、KT-01 v0 和当前工程决策。**部分技术路线已冻结供实现，实体通行与包往返仍待必要检查。** 需求文档的产品参数未被擅自更改；用户已要求直接放行实现任务，故未验证项以可配置实现继续，不冒称检查通过。
...
1. KT-01 v0 已提交 `0343e59`，INT-LT 正在隔离集成；S1 Unity 编译尚未完成。
2. W0-B 的 Unity 探针因 LicenseClient IPC 失败，1 格转角／混合接缝的实体通行仍未核验。不得把静态 0.36 净空当 S2 通过。
3. KT-02/03/06 可消费已冻结的身份／Chunk 元数据 v0，场景和公共文件仍由 INT-LT 串行保存；必要签名变更应先通知消费者。
4. S1～S4 在最终会合点最少各做一次；G4 与完整 A/B、第二作者体验后置。
