...
| P2-09 | offline_prep_integrated | `codex/p2-09-online-commander` 最终原提交 `4428650`；本地功能 `1098c9f`、一行 Windows 配置路径修正 `16b9fc3` | 出生点北侧安全节点、通讯页自由输入、受限文本 Proxy 与离线预设主题已接 NativeDemo；默认 `proxyUrl` 为空，客户端无密钥且不把回复送往 Quest/Support/Map/结局。源 Unity batch 接线/编译正常，图形 Play 14 项聚焦检查通过；`-nographics` 截图探针曾原生崩溃，图形复验通过。无真实服务/凭据或在线回复，B9 仍 not_run；Windows 未测 |
