> **当前交付/等待人工反馈：** 中文版已集成至 `c8083c1853049d189d94f3c5ea25ee006abb3cfa`，native-integration 与 de32 工作区一致，NativeDemo 唯一入口。所有执行任务 idle，Unity 槽空闲。已向用户提交中文构建和集中试玩说明，等待新版 Windows 实际通关/支援/新生/重开反馈；未测项不标通过，无自动构建或自动恢复任务。

> **简中整包交付：** `e4891ed` 完成当前 NativeDemo 及依赖文字的 FusionPixel/简体中文迁移，前序 `f3bef9d`→`a8f5910`（原译文 `8a2cb96`勿重复）。两执行任务均 idle，Unity 槽空闲；主控续派 INT 集成与交接说明，不重复验证、不自动构建。39组件字体统一、561字符无缺字、25展示状态检查/最终编译通过为执行者报告；中文 Windows 人工试玩待用户。

> **用户追加字体/中文任务：** U4 已集成 `888f1f8`，最终人工试玩版先追加 FusionPixel 全字体替换与简体中文。当前两执行：原 P1-07（4113，codex/unity-native-zh-fonts，字体/场景/Prefab/Unity）+ 原 P1-06（5b6b，codex/unity-native-zh-text，当前 NativeGame 文案与一次性序列化迁移工具）。现有代码路径与资源写入边界已分开，先字体后统一接译文；不引入通用本地化框架，不扩历史原型，不操作源字体项目，不自动构建。

> **U4 完成待 U5：** 主线 `d29b004` 已交付并 idle，Unity 槽空闲；顺序增量 `90a3ab9`→`993568c`→`3f7bebd`→`d29b004`。支援 `d24ecee` 等价于 `3f7bebd`，勿重复。主控续派 INT 精确集成与同份手动构建说明，随后用户集中 Windows 人工试玩；暂停新增功能，只处理实际阻断/严重反馈。U4 必要编译/组件链路通过为执行者报告，人工键鼠/完整 Windows/五分钟节奏未测；旧 G1/G2/G3 不宣称通过。

> **U3 Boss 交付：** `9def779` 已保存，两种攻击、核心窗口和近身 E；执行任务已 idle。U3 主线已获授权在安全点 cherry-pick 该提交并用真实 Boss 接通场景；接口不重复合入。Boss 自身未跑 Unity，统一随主线必要编译/启动检查，不重复 U2。

> **首次 Windows 构建反馈：** 用户已明确确认 `66ea557` 可启动、移动、切换开火、操作终端与重开，无错误。U2 实际构建检查点完成（用户报告）；U3 完整主线/Boss 和 U4 新功能仍待实现/试玩，不重复 U2 全量验证。

> **U3 当前：** U2 集成 `66ea557` 已含 ECA 修正版，可供用户手动构建；工作区 de32，NativeDemo 唯一入口。当前执行 U3 主线（原 P1-07，4113，Unity 槽）+ U3 Boss（原 P1-05，b785，codex/unity-native-boss）。Boss 独占 Boss/、NativeCombat、NativeProjectile；主线独占场景/规则/任务/对白/UI。小接口 `4191c87` 已交双方；主线预留中心 (23,0)、半尺寸 (8,7) 竞技区。INT 空闲，无自动打包。

> **原生最新调度：** U1 提交 `91ca855` 已接入本关 ECA 和职责拆分，源码安全点 `988b163` 保留。Unity 编译及组件/定位驱动 ECA 冒烟通过（执行者报告），真实键鼠、Windows、完整玩家通关未测。INT 正合入 ECA 修正版并更新 U2 手动构建说明；U1 已续派 U3 主线，独占 Unity/场景/规则/任务/对白，待 INT 结束后第二槽给 Boss。当前执行并发 2。

> **架构理解已纠正（2026-09-23）：** ECA 与 Entity/Map/Actor/Combat/Interaction/Quest/Dialogue/Level 分工保留；只取消独立引擎/适配/双宿主等要求，不恢复旧 P1。U1 已收到即时调整：复用 NativeDemo，收敛 Run，接入真实终端→ECA→Quest/Map/Dialogue→出口链。U0 已停止环境工作，后续仅文档保存与源码集成；Windows 用户手动构建。

> **最新职责修订：** 用户已安装 Windows 构建环境，全部 Windows 构建由用户手动处理；U0 停止模块安装/构建安排，后续仅源码集成。U1 继续开发及必要编译/启动冒烟。此项覆盖下文先前 U0 环境派发。
